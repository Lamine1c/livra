...
// Bounce livreur → app. Auto-deeplink ~300ms (comme meta-callback) + bouton de secours
// et lien /telecharger. JAMAIS de formulaire web, JAMAIS de PII : c'est un simple relais.
export default function RejoindreClient() {
  const searchParams = useSearchParams();
  const attempted = useRef(false);

  const params = searchParams.toString();
  const deepLink = `livramobile://rejoindre${params ? `?${params}` : ""}`;
